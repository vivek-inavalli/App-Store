...
class AppStore extends Component {
  state = {
    activecategory: tabsList[0].tabId, // Default to first tab (Social)
    inputChangin: '', // For handling search input
  }

  // Function to get filtered apps based on active category and search input
  getFIlteres = () => {
    const {activecategory, inputChangin} = this.state
    return appsList.filter(
      each =>
        each.category === activecategory &&
        each.appName.toLowerCase().includes(inputChangin.toLowerCase()),
    )
  }

  // Updates inputChangin based on search input
  onChangeInput = event => {
    this.setState({inputChangin: event.target.value})
  }

  // Updates active category based on clicked tab
  onClickedName = tabId => {
    this.setState({activecategory: tabId})
  }

  render() {
    const {activecategory, inputChangin} = this.state
    const filtreedList = this.getFIlteres()
        ...
        <ul className="tabs-list">
          {tabsList.map(each => (
            <TabItem
              key={each.tabId}
              details={each}
              onClickedName={this.onClickedName}
              isActive={activecategory === each.tabId}
            />
          ))}
        </ul>

        {/* Display filtered apps based on category and search */}
        <div className="mini">
          <ul className="lists">
            {filtreedList.map(each => (
